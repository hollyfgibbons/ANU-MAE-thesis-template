...
% ===================== Graphics and Layout =====================
\usepackage{graphicx}
\usepackage{tikz}           % used for the placement logo placeholder on the title page
\usepackage{rotating}
\usepackage{pdfpages}
...
% etoc's [lof,lot] option conflicts with hyperref on LaTeX 2025-11-01 because
% it redefines \addcontentsline via \def at \begin{document}, bypassing the
% kernel hook system.  The reimplementation below avoids that:
%   1. A hook mirrors each lof/lot \addcontentsline call into the .toc file
%      as a custom 'lofitem'/'lotitem' entry.
%   2. \locallistoffigures / \locallistoftables read these back per-chapter
%      using etoc's \localtableofcontents, hiding all standard TOC entries.
\makeatletter
\AddToHookWithArguments{cmd/addcontentsline/before}[lof-to-toc]{%
  \def\@ltl@ext{#1}%
  \ifx\@ltl@ext\Etoc@lofext
    \addtocontents{toc}{\protect\contentsline{lofitem}{#3}{\thepage}{}}%
  \else
    \ifx\@ltl@ext\Etoc@lotext
      \addtocontents{toc}{\protect\contentsline{lotitem}{#3}{\thepage}{}}%
    \fi
  \fi
}
\makeatother

\etocsetlevel{lofitem}{6}
\etocsetlevel{lotitem}{6}
...
\renewcommand{\locallistoffigures}{%
  ...
  \etocsetstyle{lofitem}{}{}{\noindent\etocname\dotfill\etocpage\par}{}%
...
%
% Option C — Overleaf native Zotero sync:
%   In Overleaf: Menu → Sync → Zotero
%   (requires a linked Zotero account in Overleaf account settings)
%
% CITING REFERENCES
